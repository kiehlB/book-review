...
import usePostLike from '@/components/post/hooks/use-post-like';
import useGetPostView from '@/components/post/hooks/use-get-post-view';
import ProfileIcon from '@/components/svg/profile';
import PawButton from '@/components/like-button';
import { Suspense } from 'react';
import useGetPost from '../write/hooks/use-get-single-post';

export type PostProps = {
  header_url: any;
};

function PostPageView({ header_url }: PostProps) {
  const pathSegments = header_url.split('/');
  const desiredSegment = pathSegments[pathSegments.length - 1];
  const { singlePostError, singlePostData } = useGetPost(desiredSegment) as any;
  const router = useRouter();

  const dispatch = useDispatch();

  const insertID = setHeadingId(singlePostData?.post?.body) as any;
  ...
  const getPostData = () => {
    ...
    dispatch(
      getPostTags(
        singlePostData?.post?.tags?.map((e: { tag: { name: any } }) => e?.tag?.name),
      ),
    );
                      ...
                      <div className="mx-auto mb-[1rem] flex max-w-[812.5px] rounded bg-[#F8F9FA] px-8 py-8 shadow dark:bg-[#2b2d31] ssm:flex-col">
                        ...
                        <div className="ml-8 flex flex-col ssm:ml-0 ssm:mt-2">
                          ...
                          <div className="mt-2 text-base font-semibold text-[#495057] dark:text-[#ececec]">
                            저자:
                            {singlePostData?.post?.bookInfo?.bookAuthors?.map(
                              (e: any) => e,
                            )}
                          </div>
                        </div>
