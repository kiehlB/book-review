...
import { SetStateAction, useState, useSyncExternalStore } from 'react';
...
import useWhoAmI from './auth/hooks/use-whoami';
import { useReduxState } from '@/store/use-redux-state';
...
function Header({
  ...
  //   () => store.getState().auth, // 클라이언트에서 사용되는 getSnapshot 함수

  //   () => store.getState().auth, // 서버에서 사용되는 getServerSnapshot 함수
  // );
